#include "params.hpp"
#include "indices.cuh"
#include <cstdio> // Ensure you include this header for printf to work on device
#include <cuda.h>
...
__device__ float ssim(const uchar* patch1, const uchar* patch2, int width, int height) {
    float mean1 = 0.0f, mean2 = 0.0f, var1 = 0.0f, var2 = 0.0f, covar = 0.0f;
    // Calculate means
    ...
    for (int i = 0; i < width * height; ++i) {
        var1 += (patch1[i] - mean1) * (patch1[i] - mean1);
        var2 += (patch2[i] - mean2) * (patch2[i] - mean2);
        covar += (patch1[i] - mean1) * (patch2[i] - mean2);
    }
    ...
    float numerator = (2 * mean1 * mean2 + C1) * (2 * covar + C2);
		...
		for(int x = from.x; x <= (int)params.n; x += num_warps)
		{
			...
			for(uint i = tid; i < p_rectangle_width && p_rectangle_start+i < image_dim.x && q_rectangle_start+i < image_dim.x; i+=warpSize)
			{
				...
				for(uint iy = 0; iy < params.k; ++iy)
				{
					...
					// dist += ssim(&s_image_p[idx2(i, iy, p_rectangle_width)],&image[idx2(q_rectangle_start + i, q.y + iy, image_dim.x)],p_rectangle_width, params.k);
				}
				s_diff[i] = dist;
			}
