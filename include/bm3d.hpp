...
extern "C" void run_block_matching(
    ...
    ushort* stacks,                   // For each reference patch contains addresses of similar patches
    ...
    const float* block_variances      // Pass this as const float* instead of std::vector<double>
);
...
private:
	...
	*/
	void first_step(std::vector<uchar*> & denoised_image, int width, int height, int channels, uint* sigma, const float* blockVariances)
	{	
		//image dimensions
	...
	void second_step(std::vector<uchar*> & denoised_image, int width, int height, int channels, uint* sigma, const float* blockVariances)
	{	
		//Image dimensions
		const uint2 image_dim = make_uint2(width,height);
...
public:
	...
	*/
	void denoise_host_image(uchar *src_image, uchar *dst_image, int width, int height, int channels, uint* sigma, bool two_step, const std::vector<double>& blockVariances)
	{
		Stopwatch total;
		total.start();
		...
		if (h_reserved_width == 0 || h_reserved_height == 0 || h_reserved_channels == 0 )
		    ...
		    // Allocate device memory for block variances
    	float* d_block_variances;
    	cuda_error_check(cudaMalloc(&d_block_variances, blockVariances.size() * sizeof(float)));

    	// Copy blockVariances from host to device
    	cuda_error_check(cudaMemcpy(d_block_variances, blockVariances.data(), blockVariances.size() * sizeof(float), cudaMemcpyHostToDevice));
    	...
    	first_step(d_denoised_image, width, height, channels, sigma, d_block_variances);
		...
		if (two_step)
		{
			...
			p2.start();
			null_aggregation_buffers(width,height);
        	second_step(d_denoised_image, width, height, channels, sigma, d_block_variances);
			if (_verbose)
